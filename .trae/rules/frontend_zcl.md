...
- 时间戳: 本地日志使用三级标题 `### YYYY_MM_DD_HHMM`, 倒序排列 (最新在最上)。
...
- 组件与样式: 使用 Vue3 `<script setup lang="ts">`; Props 定义 Interface; 样式使用 `<style scoped>`; 严禁使用固定 `px` (1px 边框除外), 必须使用相对单位 (`rem`/`vw`/`vh`).
