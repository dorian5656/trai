    ...
    def initialize(cls, use_angle_cls: bool = True, lang: str = 'ch', use_gpu: bool = True):
        if cls._ocr is None:
            try:
                ...
                # 存储初始化参数，供 predict 时使用
                cls._use_angle_cls = use_angle_cls
    ...
    def _predict_sync(cls, image_path: str) -> List[Dict[str, Any]]:
        ...
        with cls._lock:
            try:
                ...
                # result = cls._ocr.ocr(image_path, cls=cls._use_angle_cls) 
                # 兼容旧版本调用方式，如果 cls 参数报错，尝试不传 cls 参数
                try:
                    result = cls._ocr.ocr(image_path, cls=cls._use_angle_cls)
                except TypeError as e:
                    if "unexpected keyword argument 'cls'" in str(e):
                        logger.warning("PaddleOCR.ocr() 不支持 cls 参数，尝试降级调用")
