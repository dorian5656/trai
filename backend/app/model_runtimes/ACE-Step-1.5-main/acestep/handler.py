...
try:
    ...
    from torch import nn, tensor, int32
    import torch

    # Helper to fix meta tensors
    def ensure_fsq_buffers(self, device):
        if hasattr(self, '_levels') and self._levels.device.type == 'meta' and hasattr(self, '_levels_list'):
            self._levels = tensor(self._levels_list, device=device, dtype=int32)
        if hasattr(self, '_basis') and self._basis.device.type == 'meta' and hasattr(self, '_levels_list'):
            levels = self._levels_list
            self._basis = torch.cumprod(tensor([1] + levels[:-1]), dim = 0, dtype = int32).to(device)

    # Save original methods
    _orig_bound = FSQ.bound
    _orig_symmetry_preserving_bound = FSQ.symmetry_preserving_bound
    _orig_indices_to_codes_internal = FSQ._indices_to_codes

    def safe_bound(self, z, *args, **kwargs):
        ensure_fsq_buffers(self, z.device)
        return _orig_bound(self, z, *args, **kwargs)

    def safe_symmetry_preserving_bound(self, z, *args, **kwargs):
        ensure_fsq_buffers(self, z.device)
        return _orig_symmetry_preserving_bound(self, z, *args, **kwargs)

    def safe_indices_to_codes_internal(self, indices):
        ensure_fsq_buffers(self, indices.device)
        return _orig_indices_to_codes_internal(self, indices)
    ...
    def safe_fsq_init(
        ...
        # Save levels list for recovery from meta device
        self._levels_list = levels

        # Force CPU first, register_buffer will move it if needed (or to meta)
        ...
        if return_indices:
            # Safe calculation of codebook_size
            prod = 1
            for l in levels: prod *= l
            self.codebook_size = prod
            
            try:
                implicit_codebook = self._indices_to_codes(torch.arange(self.codebook_size))
                if implicit_codebook.device.type == 'meta':
                    # If on meta (init_empty_weights), do not register buffer to avoid stuck on meta
                    # Fallback to dynamic computation during inference
                    self.implicit_codebook = None
                else:
                    self.register_buffer('implicit_codebook', implicit_codebook, persistent = False)
            except RuntimeError:
                # If failed, fallback to dynamic computation
                self.implicit_codebook = None
    ...
    FSQ.__init__ = safe_fsq_init
    FSQ.bound = safe_bound
    FSQ.symmetry_preserving_bound = safe_symmetry_preserving_bound
    FSQ._indices_to_codes = safe_indices_to_codes_internal

    # Helper to fix meta tensors for ResidualFSQ
    def ensure_residual_fsq_buffers(self, device):
        if hasattr(self, 'scales') and self.scales.device.type == 'meta' and hasattr(self, '_scales_list'):
            self.scales = torch.stack([s.to(device) for s in self._scales_list])
            
        if hasattr(self, 'soft_clamp_input_value') and self.soft_clamp_input_value is not None:
             if self.soft_clamp_input_value.device.type == 'meta':
                  # Recompute soft_clamp_input_value
                  levels_tensor = tensor(self.levels, device=device)
                  val = 1 + (1 / (levels_tensor - 1))
                  self.soft_clamp_input_value = val

    # Save original ResidualFSQ methods
    _orig_residual_forward = ResidualFSQ.forward
    _orig_get_codes = ResidualFSQ.get_codes_from_indices

    def safe_residual_forward(self, x, *args, **kwargs):
        ensure_residual_fsq_buffers(self, x.device)
        return _orig_residual_forward(self, x, *args, **kwargs)

    def safe_get_codes(self, indices):
        ensure_residual_fsq_buffers(self, indices.device)
        return _orig_get_codes(self, indices)

    # Patch ResidualFSQ.__init__
    def safe_residual_fsq_init(
        self,
        *,
        levels: list[int],
        num_quantizers,
        ...
        **kwargs
        ...
        # assert (levels_tensor > 1).all() # Skipped
        ...
        self._scales_list = scales # Save for recovery
        ...
        if bound_hard_clamp:
             if soft_clamp_input_value is None:
    ...
    ResidualFSQ.__init__ = safe_residual_fsq_init
    ResidualFSQ.forward = safe_residual_forward
    ResidualFSQ.get_codes_from_indices = safe_get_codes
    
    logger.info("Monkeypatched FSQ and ResidualFSQ to skip meta tensor assertion and fix buffers")
except ImportError:
    pass
# --- Monkeypatch end ---

# Force disable Flash Attention to avoid meta tensor issues with SDPA/Eager fallback
# os.environ["ACESTEP_DISABLE_FLASH_ATTN"] = "1" 
    ...
    def initialize_service(
        ...
        try:
            ...
            if device in ["cuda", "xpu"]:
                ...
                self.dtype = torch.float32 # Try float32 to avoid NaNs with flash-attn
            else:
            ...
            env_checkpoint_dir = os.environ.get("ACESTEP_CHECKPOINTS_DIR")
            checkpoint_dir = env_checkpoint_dir if env_checkpoint_dir else os.path.join(actual_project_root, "checkpoints")
            ...
            if os.path.exists(acestep_v15_checkpoint_path):
                ...
                if use_flash_attention:
                    # Explicitly try importing flash_attn if requested for CUDA
                    # Fix: check torch.cuda.is_available() first, and ignore 'device' string if it is "auto" but maps to cuda
                    is_cuda = (device == "cuda") or (device == "auto" and torch.cuda.is_available())
                    ...
                    if is_cuda:
                        try:
                            ...
                            logger.warning(
                                f"[initialize_service] Flash attention requested but 'flash_attn' package not found for device={device}. "
                                "Falling back to SDPA."
                            )
                            attn_implementation = "sdpa"
                    else:
                        # For non-CUDA devices, flash_attn is usually not available/supported
                        logger.warning(
                            f"[initialize_service] Flash attention requested but not supported for device={device}. "
                            "Falling back to SDPA."
                        )
                        attn_implementation = "sdpa"
                else:
