...
### 💻 客户端_2026_02_28_1717
- **修复(client)**: 优化媒体工具页面 (`media_tools_page.py`) 中的文件句柄管理, 修复潜在的资源泄漏问题.
- **优化(client)**: 增强视频转 GIF 功能的异常处理与临时文件清理逻辑.

### 🛠️ 后端_2026_02_28_1636
- **修复(ai)**: 修复文生图 (`generate_image`) 接口权限校验逻辑, 恢复 `user_id` 正常获取, 确保历史记录正确入库.
- **测试(ai)**: 验证复杂提示词 (Z-Image-Turbo) 生成全流程, 确认 Prompt 和 URL 准确记录至数据库.
- **环境**: 确认 `trai_31014_whf_dev_20260227` 环境下 AI 图像/视频服务运行稳定.
...
- **修复(backend)**: 修复 Wan2.1 视频生成模型加载 Meta Tensor 错误, 确保模型正确初始化.
- **调整(backend)**: 调整依赖版本以解决参数兼容性问题, 移除冗余测试代码.
