...
- **🖼️ 图像工具箱**: 提供图片格式转换、图片转 ICO、图片压缩到指定大小、图片尺寸调整等功能。
- **🎨 AI 创作**:
  - **文生图**: 调用后端 API 生成高质量图像。
  - **图片解析**: 基于多模态模型解析图片内容。
- **🛠️ ModelScope 工具**: 本地模型的上传与社区模型的下载。
...
> **注意**: 首次运行前，请确保 将 `config.json.example` 修改为 `config.json` 并将其中的 API 地址指向正确的后端服务 IP (默认为 `127.0.0.1` 或 `localhost`)和端口（默认为5789）。
...
- **新增**: 新增AI 文生音乐服务
- **新增**: 基于API接口实现功能并优化体验（使用模型: ACE-Step/Ace-Step1.5）
- **优化**: 优化AI 文生视频的缓存机制
- **优化**: 优化AI 文生图的页面布局

### 2026_02_27_1352
- **新增**: 新增AI 文生视频模块
- **新增**: 基于API接口实现功能并优化体验（使用模型: Wan2.1-T2V-1.3B）

### 2026_02_26_1721

- **新增**: 新增语音服务模块，包括语音服务健康检查和音频转文字功能。
- **新增**: 基于API接口，实现上述功能并优化。
- **优化**: 实现复制转换结果功能。

### 2026_02_26_1002

- **优化**: 系统设置模块，将当前版本的格式修改为与最新版本相同的时间戳格式，避免了语义化版本号（如 1.3.0 ）与时间戳版本号（如 202602251730）混用导致的比较逻辑复杂逻辑判断的问题，同时优化版本比较逻辑。
- **优化**: 优化exe打包脚本的版本自动匹配settings_page.py中的CURRENT_VERSION，确保了 代码中定义的版本 与 构建发布后的版本 始终保持一致。
- **优化**: 解决图片工具箱加载源图片时，部分图片无法加载的问题。

### 2026_02_25_1555

- **优化**: 优化图片工具箱模块的布局，整体更加简洁、美观
- **优化**: 基于API接口，完善片格式转换、图片转 ICO、图片尺寸调整等功能已完成，图片压缩到指定大小API接口存在问题，待解决。

### 2026_02_24_1730

- **优化**: 优化文档工具箱的布局
- **新增**: 新增图像工具箱模块，当前功能包括：图片格式转换、图片转 ICO、图片压缩到指定大小、图片尺寸调整
- **优化**: 优化图片工具箱模块的布局
